...
class Translator(nn.Module):
    ...
    def _model_decode(self, trg_seq, enc_output, src_mask):
        ...
        dec_output, *_ = self.model.decoder(trg_seq, trg_mask, enc_output, src_mask)  # beam_size x len x model_d
        return F.softmax(self.model.trg_word_prj(dec_output), dim=-1)  # beam_size x len x d_vocab

    def _get_init_state(self, src_seq, src_mask):
        beam_size = self.beam_size

        enc_output, *_ = self.model.encoder(src_seq, src_mask)   # 1 x len x embedding_d
    ...
    def translate_sentence_with_beam_search(self, src_seq):
        ...
        with torch.no_grad():
            ...
            ans_idx = 0   # default
            '''
            这一块有个问题，那就是beam_search时，如果有句子已经有eos了却不会终止。任务会等到每个句子都有eos才终止。
            那么已有eos的句子仍然会不断生成新句子。          
            '''
