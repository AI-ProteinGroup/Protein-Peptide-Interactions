...
class ProteinGraphDataset(data.Dataset):
    ...
    def _featurize_as_graph(self, protein):
        ...
        with torch.no_grad():
            ...
            label =  torch.tensor(protein['label'])
            pep_feature =  torch.tensor(np.array(protein['pep_feature']), dtype=torch.float)
            pep_mask=  torch.tensor(np.array(protein['pep_mask']))
            pro_mask=  torch.tensor(np.array(protein['pro_mask']))
            ...
            mask = torch.isfinite(coords.sum(dim=(1,2)))#判断是否是无穷
            coords[~mask] = np.inf
        ...
        data = torch_geometric.data.Data(x=X_ca, seq=seq, name=name,
                                         ...
                                         label = label,pep_feature =pep_feature,
                                         pep_mask=pep_mask,pro_mask=pro_mask)
        return data
