...
def argparser():
    ...
    parser.add_argument('--d_word_vec',  type=int, default='512',help='d_word_vec')
...
def train(args,device,train_loader,test_loader,optimizer,epoch_num,learningrate,network,cross_number):
        ...
        with open('./save/evalue_result.txt', 'w') as f:
            f.write('Epoch \t Time(sec) \t Loss_train \t Accuracy \t Precision \t Recall \t Sensitivity \t Specificity \t MCC \t F1-score \t Q9 \t PPV \t NPV \t TP \t FP \t TN \t FN \t ROC_AUC\n')
        ...
        for epoch in range(start_epoch,epoch_num):  
            ...
            for data in t:
                ...
                pro_features ,pro_seq,pro_mask, pep_feats,pep_mask,label_two= data
                pro_features  = pro_features.to(device)
                ...
                output,_ =model(pro_features ,pro_mask, pep_feats,pep_mask)
                # print(type(output))
                #输出是多肽的时候计算loss
        ...
        #     #后面先注释
            ...
            for data in t1:
                ...
                with torch.no_grad():
                    ...
                    output,predicted_spec=model( pro_features ,pro_mask, pep_feats,pep_mask)
            ...
            for data in t2:
                ...
                with torch.no_grad():
                    ...
                    output,predicted_spec= model_test( pro_features ,pro_mask, pep_feats,pep_mask)
                    loss = criterion(output,label)
...
# 设置随机数种子
    ...
    for cross_number in range(1):
        ...
        train_path = "../new_data/esmif6A_mynet.jsonl"
        valid_path = "../new_data/esmif6A_mynet.jsonl"
        test_path = "../new_data/esmif6A_mynet.jsonl"
        split_path = "../new_data/pocketonly_split_23.jsonl"

        #读取数据
        print('----------Begin to load dataset')
        train_data = MyDataset(train_path,split_path,"train")

        

        valid_data = MyDataset(valid_path,split_path,"validation")
        train_data = ConcatDataset([train_data , valid_data ])

        train_loader = DataLoader(train_data,batch_size=batch_size,shuffle=shuffle,pin_memory=True)
        
        test_data = MyDataset(test_path,split_path,"test")
        ...
        train(args,device,train_loader,test_loader,'Adam',200,0.000005,'MyNet',cross_number)
